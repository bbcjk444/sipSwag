import "./Game_ch1_play_05.css";

const Game_ch1_play_05 = () => {
  return (
    <div className="ch1-1">
      <img className="icon" alt="" src="../1-5@2x.png" />
      <div className="group-div">
        <div className="rectangle-div" />
        <img className="group-icon" alt="" src="../group-345.svg" />
        <img className="group-icon1" alt="" src="../group.svg" />
        <img className="group-icon2" alt="" src="../group1.svg" />
        <img className="vector-icon" alt="" src="../vector.svg" />
        <div className="rectangle-div1" />
        <div className="rectangle-div2" />
        <div className="rectangle-div3" />
        <div className="rectangle-div4" />
        <div className="rectangle-div5" />
        <div className="rectangle-div6" />
        <div className="rectangle-div7" />
        <div className="rectangle-div8" />
        <div className="rectangle-div9" />
        <div className="div">
          번호를 내림차순으로 터치해서 수강을 신청해주세요!
        </div>
      </div>
      <div className="rectangle-div10" />
      <img className="finish-1-icon" alt="" src="../finish-1@2x.png" />
    </div>
  );
};

export default Game_ch1_play_05;
